...
def assign_labels(input_string, labels):
    ...
    for token in words_with_punctuation:
        if re.match(r'[\u0E00-\u0E7F\w]+', token): #if re.match(r"\w+'?\w*", token):
...
def tokenize_text_with_labels(
    ...
    min_words=4,
    max_words=70, # increased because of the new tokenizer
    ...
    for i, (word, label) in enumerate(zip(words, labels)):
        ...
        '''if explicit_words_length:
            tmp = []
            for token in tokens:
                for i in range(len(token)): # the 2 * was just to add a bunch of embeddings and see if predictions improve
                    tmp.append(token)
            tokens = tmp'''
...
class TokenTaggingDataset(Dataset):
    def __init__(
        ...
        self.debug = debug
        self.explicit_words_length = explicit_words_length
        ...
        for text, labels_per_word in tqdm(
            ...
            result = tokenize_text_with_labels(
                ...
                explicit_words_length=self.explicit_words_length,
            )
...
'''class SubtitlesDataset(Dataset):
    def __init__(self, file_path, tokenizer,invalid_label=-1):
        self.file_path = file_path # this should be a path to a text file
        self.tokenizer = tokenizer
    ...
    def __getitem__(self, idx):
        tokens = [self.tokenizer.tokenize(line) for line in self.data[idx]]
        input_ids_1 = [self.tokenizer.encode(line) for line in self.data[idx]]
        input_ids = input_ids_1 #pad_sequence([torch.tensor(seq) for seq in input_ids_1], batch_first=True, padding_value=0)
        mask = [1 if np.all(label != self.invalid_label) else 0 for label in input_ids_1]
        word_to_tokens = [(word, ids) for word, ids in zip(tokens, input_ids_1)]
        return {"input_text": self.data[idx],
                    "tokenized_text": tokens,
                    "original_labels": self.data[idx],
