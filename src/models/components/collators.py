...
def vector_collate_fn(batch, eos_token_id, invalid_label=-1):
    ...
    for i, item in enumerate(batch):
        ...
        # loss_mask = (token_ids != eos_token_id).long()
...
def collate_batch(batch, pad_token_id):
    ...
    for i, item in enumerate(batch):
        ...
        # Assuming "original_labels" is a list of labels for each word
        labels_tensor = torch.cat([labels_tensor, torch.tensor(item["labels"], dtype=torch.long)])
