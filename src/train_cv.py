...
def train(cfg: DictConfig) -> Tuple[dict, dict]:
    ...
    #     )
    if cfg.get("train"):
        log.info("Starting k-fold cross-validation training!")
        test_results = []
        for fold in range(datamodule.hparams.n_splits):
            log.info(f"Training on fold {fold+1}/{datamodule.hparams.n_splits}")
            datamodule.setup()
            trainer.fit(
                model=model_task, datamodule=datamodule, ckpt_path=cfg.get("ckpt_path")
            )
            log.info(f"Testing on fold {fold+1}/{datamodule.hparams.n_splits}")
            test_metrics = trainer.test(model=model_task, datamodule=datamodule)
            test_results.append(test_metrics[0])  # Save fold results
            datamodule.next_fold()
        # Compute mean and standard deviation across folds
        metric_keys = test_results[0].keys()
        final_results = {}
        for key in metric_keys:
            values = [fold_result[key] for fold_result in test_results]
            final_results[key] = {
                "mean": np.mean(values),
                "std": np.std(values),
                "all_folds": values,
        ...
        log.info(f"Cross-validation results: {final_results}")

        # Save the final results
        with open(os.path.join(cfg.paths.output_dir, "cv_results.json"), "w") as f:
    ...
    if cfg.get("test"):
        ...
        if ckpt_path == "":
            if cfg.get("ckpt_path"):
                log.warning("Using ckpt_path from cfg for testing...")
